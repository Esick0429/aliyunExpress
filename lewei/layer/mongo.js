...
exports.findAll = async function (url,database, tablename, data) {
...
exports.find = async function(url,database,collectionName,json,sort){
...
//数据分页
exports.pageing = async function (url,database, tablename,data, now, num, sort) {
    const conn = await connect(url);
    const dbbase = conn.db(database);
    let total = await dbbase.collection(tablename).find(data).count();
...
//插入单条数据
exports.insertOne = async function (url,database, tablename, data) {
    const conn = await connect(url);
    const dbbase = conn.db(database);
...
//删除单条数据
exports.dRole = async function (url,database, tablename, data, set) {
    const conn = await connect(url);
    const dbbase = conn.db(database);
...
//更新
exports.updateInfo = async function (url,database, tablename, data, set, callback) {
    const conn = await connect(url);
    const dbbase = conn.db(database);
    let res = dbbase.collection(tablename).updateOne(data, set)
    conn.close
