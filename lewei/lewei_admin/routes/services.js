...
exports.login = (req, res) => {
    ...
    db.findAll('lewei_admin', 'user_info', {
        deleted: false,
        phone: userphone
    }, function (total, data) {
        console.log(data[0]);
...
exports.router = (req, res) => {
    ...
    db.find('lewei_admin', 'user_info', {
        deleted: false,
        user_id: user.userId
    }, {
        create_time: 1
    }, function (user) { //查询roleId
        console.log(user[0]);
        db.find('lewei_admin', 'role_info', {
            deleted: false,
            _id: ObjectId(user[0].role_id)
        }, {
            create_time: 1
        }, function (role) { //通过roleId查询routerId
            console.log(role[0]);
            let arr = role[0].router_id
            var data = []
            const pp = new Promise((resolve, reject) => {
                for (var i = 0; i < arr.length; i++) {
                    db.find('lewei_admin', 'router_info', {
                        _id: ObjectId(arr[i])
                    }, {
                        create_time: 1
                    }, function (res) { //查找router——config
                        console.log(res);
                        data.push(res[0])
...
exports.getUser = (req, res) => {
    ...
    db.pageing('lewei_admin', 'user_info', {
        deleted: false
    }, pageIndex, pageSize, {
        create_time: -1
    }, function (total, reslut) {
...
exports.dUser = (req, res) => { //删除
    ...
    console.log(user_id);
    db.updateInfo("lewei_admin", "user_info", {
        'user_id': user_id
    }, {
        $set: {
            'deleted': true,
